...
local function ModifiedString(string)
    ...
    for pattern in string_gmatch(string, "(Q.*')") do newString = newString:gsub(pattern, '|cffC41F3B'..pattern:gsub("'", '')..'|r') end
    for pattern in string_gmatch(string, "(A.*')") do newString = newString:gsub(pattern, '|cffc0c0c0'..pattern:gsub("'", '')..'|r') end
