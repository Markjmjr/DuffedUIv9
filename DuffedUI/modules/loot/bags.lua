...
function Stuffing:SlotNew(bag, slot)
	...
	if not ret.frame then
		...
		if not ret.frame.Corrupt then
			ret.frame.Corrupt = ret.frame:CreateTexture(nil, 'OVERLAY')
			ret.frame.Corrupt:SetAtlas("Nzoth-inventory-icon")
			ret.frame.Corrupt:SetSize(35, 35)
			ret.frame.Corrupt:SetPoint('TOPLEFT', 1, -1)
			ret.frame.Corrupt:Hide()
		end
