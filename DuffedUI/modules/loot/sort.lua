...
local B = D:NewModule("InventorySort", "AceHook-3.0", "AceEvent-3.0", "AceTimer-3.0")
	...
	for i = 1, select("#", ...) do
		...
		for bagType, sortedBags in pairs(bagCache) do
			if bagType ~= 'Normal' then
				...
				B.Fill(bagCache.Normal, sortedBags, C['bags'].SortInverted)
				B.Sort(sortedBags, nil, C['bags'].SortInverted)
				wipe(sortedBags)
			end
		end
		...
		if bagCache.Normal then
			...
			B.Sort(bagCache.Normal, nil, C['bags'].SortInverted)
