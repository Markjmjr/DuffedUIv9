...
local function OnEnter(self)
	...
	GameTooltip:AddLine(L['dt']['totalmemusage'])
	GameTooltip:AddLine(' ')

	local Bandwidth = GetAvailableBandwidth()
	if(Bandwidth ~= 0) then
		GameTooltip:AddDoubleLine(NameColor .. L['dt']['bandwidth'] .. '|r', format(ValueColor .. BandwidthString .. '|r', Bandwidth))
		GameTooltip:AddDoubleLine(NameColor .. L['dt']['download'] .. '|r', format(ValueColor .. PercentageString .. '|r', GetDownloadedPercentage() * 100))
		GameTooltip:AddLine(' ')
	end
...
DataText:Register(L['dt']['memory'], Enable, Disable, Update)